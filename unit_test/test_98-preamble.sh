...
echo "#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
echo "# unit_test: 98-preamble.sh -- exercises preamble support (test_YYY_XX-NAME.sh)"

echo
echo "# --help mentions the preamble naming convention"

run_test "./unitt -h" "0" "$(escape_expected "test_YYY_XX-NAME.sh")"
...
register_pre_file "unit_test/test_777_77-pre_smoke.sh"
cat > unit_test/test_777_77-pre_smoke.sh <<'EOF'
...
register_pre_file "unit_test/test_779_00-pre_zero.sh"
cat > unit_test/test_779_00-pre_zero.sh <<'EOF'
...
register_pre_file "unit_test/test_771_0-zero_tag.sh"
cat > unit_test/test_771_0-zero_tag.sh <<'EOF'
...
register_pre_file "unit_test/test_888_88-pre_missing.sh"
cat > unit_test/test_888_88-pre_missing.sh <<'EOF'
...
PREAMBLE_08_OK="yes"
EOF
register_pre_file "unit_test/test_809_08-leading_zero.sh"
cat > unit_test/test_809_08-leading_zero.sh <<'EOF'
...
# any non-numeric preamble tag before discovery returns.
...
:
EOF
register_pre_file "unit_test/test_123456789012345678_01-edge18.sh"
cat > unit_test/test_123456789012345678_01-edge18.sh <<'EOF'
run_test "true" "0" ".*"
EOF
run_test "./unitt -u edge18 -l" "0" "unit:.123456789012345678.-.edge18."

# 19-digit id: rejected.
register_pre_file "unit_test/test_1234567890123456789_01-over19.sh"
cat > unit_test/test_1234567890123456789_01-over19.sh <<'EOF'
run_test "true" "0" "should-never-run"
EOF
run_test "./unitt -l" "1" "$(escape_expected "exceeds 18 digits")"

# 19-digit preamble tag: same guard fires for the tag.
register_pre_file "unit_test/test_42_1234567890123456789-bigtag.sh"
cat > unit_test/test_42_1234567890123456789-bigtag.sh <<'EOF'
run_test "true" "0" "should-never-run"
EOF
run_test "./unitt -l" "1" "$(escape_expected "exceeds 18 digits")"
# Remove the over-length files before the next test so its error message
# is the one that wins.
rm -f unit_test/test_1234567890123456789_01-over19.sh \
      unit_test/test_42_1234567890123456789-bigtag.sh

# Non-numeric preamble tag: rejected (regex would allow letters in the
# tag position but the arithmetic sort key wouldn't).
